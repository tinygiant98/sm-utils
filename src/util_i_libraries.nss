...
/// 
/// `util_i_libraries.nss` holds functions for interacting with libraries. Several other
/// utility scripts from sm-utils are required.  Importing all sm-utils scripts is
/// highly recommended.
/// 
...
/// ## Awaiting a Library
/// 
/// If library load order is important, libraries can await other libraries
/// before they are loaded.  Each normal loading function listed above has a sibling
/// await function available that loads a library with the same criteria, but first
/// awaits the loading of a specified set of libraries.
/// 
/// **Warning** If a prerequisite library is not found or never loaded, the awaiting
/// library will never load if not loaded via another method.
/// 
/// **Note:** When using a prerequisite list, if the awaiting library is found in the
/// prerequisite list, the awaiting library will be ignored and will not load until
/// all other libraries in the prerequisite list are loaded.
/// 
/// ```c
/// // Await a single library
/// AwaitLibrary("my_l_await", "my_l_library");
/// 
/// // Await a CSV list of library scripts
/// AwaitLibraries("my_l_await,my_p_await", "my_l_library");
/// 
/// // Await all libraries matching a glob pattern
/// AwaitLibrariesByPattern("*_l_*", "my_l_library");
/// 
/// // Await all libraries matching a prefix
/// AwaitLibrariesByPrefix("pw_l_", "my_l_library");
/// ```
/// 
/// ## Using a Library
/// 
...
/// `main()` function yourself.  Additionally, if compiling with `nasher`,
/// `util_i_libraries` can be added to a target's `skipCompile` option to prevent
/// the error.  This pragma is not required if using `nwn_script_comp` and can
/// be removed.
/// ----------------------------------------------------------------------------
...
/// @brief Return a prepared query with the library and entry data
...
/// @brief Load a script library after another specified library is loaded.
/// @param sAwait The library to wait for before loading sLibrary.
/// @param sLibrary The library script to load after sAwait is loaded.
/// @param bForce If TRUE, will re-load the library if it was already loaded.
void AwaitLibrary(string sAwait, string sLibrary, int bForce = FALSE);
...
/// @brief Load a script library after other specified libraries are loaded.
/// @param sLibraries A CSV list of libraries to wait for before loading sLibrary.
/// @param sLibrary The library script to load after sLibraries are loaded.
/// @param bForce If TRUE, will re-load the library if it was already loaded.
void AwaitLibraries(string sLibraries, string sLibrary, int bForce = FALSE);
...
/// @brief Await loading of libraries matching glob patterns before loading sLibrary.
/// @param sPatterns CSV list of glob patterns to match.
/// @param sLibrary The library to load after all matching libraries are loaded.
/// @param bForce If TRUE, will re-load the library if it was already loaded.
void AwaitLibrariesByPattern(string sPatterns, string sLibrary, int bForce = FALSE);
...
/// @brief Await loading of libraries matching a prefix before loading sLibrary.
/// @param sPrefix The prefix for libraries to await.
/// @param sLibrary The library to load after all matching libraries are loaded.
/// @param bForce If TRUE, will re-load the library if it was already loaded.
void AwaitLibrariesByPrefix(string sPrefix, string sLibrary, int bForce = FALSE);
...
/// @private Strip file extension from a library name.
/// @param sLibrary The library name, possibly with extension.
string libraries_StripLibraryExtension(string sLibrary)
{
    return JsonGetString(JsonArrayGet(RegExpMatch("^(.+?)(?:\\.[^.]*$|$)", sLibrary), 1));
}

/// @private Retrieve sField data from library_scripts table
///     for a given sScript.
/// @param sField Field to retrieve.
/// @param sScript Script name to filter the field by.
string libraries_GetScriptFieldData(string sField, string sScript)
{
    CreateLibraryTable();
...
/// @private Generate a json array of script names with a given prefix
///     and resource type.
/// @param jArray The json array to insert the script names into.
/// @param sPrefix The prefix matching the scripts to find.
/// @param nResType The resource type to search for scripts in.
json libraries_GetScriptsByPrefix(json jArray, string sPrefix, int nResType)
{
    int i;
    string sScript;
...
/// @private Load any libraries that are awaiting sLibrary to be loaded.
/// @param sLibrary Library file that may be a prerequisite for other libraries.
void libraries_LoadAwaitingLibraries(string sLibrary)
{
    CreateLibraryTable();

    string sQuery = r"
        DELETE FROM library_awaits
        WHERE sAwaiting = @sLibrary
        RETURNING sAwaiter;
    ";
    sqlquery sql = SqlPrepareQueryModule(sQuery);
    ...
    while (SqlStep(sql))
    {
        string sAwaiter = SqlGetString(sql, 0);
        string sQuery = r"
            SELECT EXISTS(
                SELECT 1 FROM library_awaits
                WHERE sAwaiter = @sAwaiter
            );
        ";
        sqlquery sql = SqlPrepareQueryModule(sQuery);
        SqlBindString(sql, "@sAwaiter", sAwaiter);
        
        if (SqlStep(sql) ? !SqlGetInt(sql, 0) : TRUE)
            LoadLibrary(sAwaiter);
    }
}
...
void CreateLibraryTable(int bReset = FALSE)
{
    if (!bReset && GetLocalInt(GetModule(), "LIBRARY_TABLES_INITIALIZED"))
        return;

    if (bReset)
    {
        SqlExecModule("DROP TABLE IF EXISTS library_libraries;");
        SqlExecModule("DROP TABLE IF EXISTS library_scripts;");
        SqlExecModule("DROP TABLE IF EXISTS library_awaits;");
    }

    Debug((bReset ? "Forced redefinition of" : "Created") + " library tables");
    ...
    SqlCreateTableModule("library_libraries", r"
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sLibrary TEXT NOT NULL UNIQUE
    ");

    SqlCreateTableModule("library_awaits", r"
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sAwaiting TEXT NOT NULL,
        sAwaiter TEXT NOT NULL
    ");

    SetLocalInt(GetModule(), "LIBRARY_TABLES_INITIALIZED", TRUE);
}
...
void AddLibraryScript(string sLibrary, string sScript, int nEntry = 0)
{
    ...
    string sQuery = r"
        INSERT INTO library_scripts (sLibrary, sScript, nEntry)
    ...
    Debug("Registered library script " + sScript + " in " + sLibrary +
            (nEntry != 0 ? " at entry " + IntToString(nEntry) : ""));
}

string GetScriptLibrary(string sScript)
{
    return libraries_GetScriptFieldData("sLibrary", sScript);
}

int GetScriptEntry(string sScript)
{
    return StringToInt(libraries_GetScriptFieldData("nEntry", sScript));
}
...
sqlquery GetScriptData(string sScript)
{
    ...
    string sQuery = r"
        SELECT sLibrary, nEntry FROM library_scripts
        WHERE sScript = @sScript;
    ";
    sqlquery sql = SqlPrepareQueryModule(sQuery);
    SqlBindString(sql, "@sScript", sScript);
...
int GetIsLibraryLoaded(string sLibrary)
{
    ...
    string sQuery = r"
        SELECT EXISTS(
            SELECT 1 FROM library_libraries
...
void LoadLibrary(string sLibrary, int bForce = FALSE)
{
    ...
    sLibrary = libraries_StripLibraryExtension(sLibrary);
    ...
    if (bForce || !GetIsLibraryLoaded(sLibrary))
    {
        ...
        string sQuery = r"
            INSERT INTO library_libraries (sLibrary)
            VALUES (@sLibrary);
        ";
        sqlquery sql = SqlPrepareQueryModule(sQuery);
        SqlBindString(sql, "@sLibrary", sLibrary);
        ...
        Notice("Library " + sLibrary + " already loaded!");

    libraries_LoadAwaitingLibraries(sLibrary);
}

void AwaitLibrary(string sAwait, string sLibrary, int bForce = FALSE)
{
    AwaitLibraries(sAwait, sLibrary, bForce);
}
...
void AwaitLibraries(string sAwaits, string sLibrary, int bForce = FALSE)
{
    CreateLibraryTable();
    ...
    if (sAwaits == "")
    {
        ...
        ";
        sqlquery sql = SqlPrepareQueryModule(sQuery);
        SqlBindString(sql, "@sAwaiter", sLibrary);
    ...
    int nLoaded, nAwaiting = CountList(sAwaits);
    int i; for (; i < nAwaiting; i++)
    {
        string sAwait = libraries_StripLibraryExtension(GetListItem(sAwaits, i));
        if (sAwait == sLibrary)
        {
            Debug("A library cannot await itself: " + sLibrary);
            continue;
        }

        if (GetIsLibraryLoaded(sAwait))
            nLoaded++;
        else
        {
            string sQuery = r"
                INSERT INTO library_awaits (sAwaiting, sAwaiter)
                VALUES (@sAwaiting, @sAwaiter);
            ";
            sqlquery sql = SqlPrepareQueryModule(sQuery);
            SqlBindString(sql, "@sAwaiting", sAwait);
            SqlBindString(sql, "@sAwaiter",  sLibrary);
            SqlStep(sql);

            Debug("Library " + sLibrary + " is awaiting library " + sAwait);
        }
    }

    if (nLoaded == nAwaiting)
        LoadLibrary(sLibrary, bForce);
}

json GetScriptsByPrefix(string sPrefix)
{
    json jScripts = libraries_GetScriptsByPrefix(JsonArray(), sPrefix, RESTYPE_NCS);
         jScripts = libraries_GetScriptsByPrefix(jScripts,    sPrefix, RESTYPE_NSS);
...
void LoadLibrariesByPattern(string sPatterns, int bForce = FALSE)
{
    ...
    Debug((bForce ? "Force loading" : "loading") + " libraries matching patterns: " + sPatterns);
...
void AwaitLibrariesByPattern(string sPatterns, string sLibrary, int bForce = FALSE)
{
    if (sPatterns == "")
    {
        AwaitLibraries("", sLibrary, bForce);
        return;
    }

    Debug((bForce ? "Forced loading" : "Loading") + " of library " + sLibrary +
        " is awaiting libraries matching patterns: " + sPatterns);

    json jPatterns  = ListToJson(sPatterns);
    json jLibraries = FilterByPatterns(GetScriptsByPrefix("") , jPatterns, TRUE);
    string sLibraries = JsonToList(jLibraries);
    AwaitLibraries(sLibraries, sLibrary, bForce);
}
...
void LoadLibrariesByPrefix(string sPrefix, int bForce = FALSE)
{
    ...
    Debug((bForce ? "Force loading" : "Loading") + " libraries with prefix: " + sPrefix);
...
void AwaitLibrariesByPrefix(string sPrefix, string sLibrary, int bForce = FALSE)
{
    ...
    Debug((bForce ? "Forced loading" : "Loading") + " of library " + sLibrary +
        " is awaiting libraries with prefix: " + sPrefix);
